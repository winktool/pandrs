...
fn test_get_categorical() {
    ...
    assert_eq!(cat.len(), 4);
    assert_eq!(cat.categories().len(), 3); // Red, Blue, Green
    ...
    // Non-categorical column test
    // In the test environment, columns might already be categorical,
    // so add a new column and verify it's not categorical
...
fn test_value_counts() {
    ...
    // Check results
    assert_eq!(counts.len(), 3);  // 3 unique values
...
fn test_modify_categorical() {
    ...
    reordered.sort_by(|a, b| {
        if a == "Blue" { return std::cmp::Ordering::Less; }
        if b == "Blue" { return std::cmp::Ordering::Greater; }
        if a == "Red" { return std::cmp::Ordering::Greater; }
        if b == "Red" { return std::cmp::Ordering::Less; }
        a.cmp(b)
    });
...
fn test_get_categorical_aggregates() {
    ...
    df.add_column("Product".to_string(), Series::new(products_str, Some("Product".to_string())).unwrap()).unwrap();
    df.add_column("Color".to_string(), Series::new(colors_str, Some("Color".to_string())).unwrap()).unwrap();
    df.add_column("Quantity".to_string(), Series::new(quantities_str, Some("Quantity".to_string())).unwrap()).unwrap();
