    ...
    fn test_standard_scaler() -> Result<(), PandRSError> {
        ...
        if let Ok(transformed_col) = transformed_df.column("feature") {
            ...
            if let Some(float_col) = transformed_col.as_float64() {
                ...
                assert!(std_dev > 0.0, "Standard deviation should be positive: {}", std_dev);
                ...
                assert!(transformed_values[0] < 0.0, "Minimum value should be negative");
                assert!(transformed_values[4] > 0.0, "Maximum value should be positive");
            ...
            if let Some(float_col) = transformed_col.as_float64() {
                ...
                for i in 0..data.len() {
                    ...
                    assert!((transformed_values[i] - expected).abs() < 1e-10, 
                            "Value at position {} differs from expected: {} vs {}", i, transformed_values[i], expected);
                }
                
                // Check min/max range
                assert!((transformed_values[0] - 0.0).abs() < 1e-10, "Minimum value should be transformed to 0.0: {}", transformed_values[0]);
                assert!((transformed_values[4] - 1.0).abs() < 1e-10, "Maximum value should be transformed to 1.0: {}", transformed_values[4]);
            } else {
                return Err(PandRSError::Column("Column is not Float64 type".to_string()));
            }
