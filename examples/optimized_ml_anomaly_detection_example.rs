...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Data generation complete: {} normal samples, {} anomaly samples", n_normal, n_anomalies);
    ...
    let mut isolation_forest = IsolationForest::new(
        100,                    // Number of trees
        None,                   // Subsampling size (default)
        None,                   // Feature subsampling (default)
        0.05,                   // Contamination rate 5%
        Some(42),               // Random seed
    );
    ...
    println!("Number of detected anomalies: {}", isolation_forest.labels().iter().filter(|&&x| x == 1).count());
    ...
    for i in 0..std::cmp::min(5, if_result.row_count()) {
        if let (Ok(Some(x)), Ok(Some(y)), Ok(Some(anomaly_score)), Ok(Some(anomaly))) = (
            ...
            println!("Row {}: x={:.4}, y={:.4}, score={:.4}, anomaly={}", 
    ...
    println!("Number of detected anomalies: {}", lof.labels().iter().filter(|&&x| x == 1).count());
    println!("First few rows of the result:");
    ...
    println!("Number of detected anomalies: {}", one_class_svm.labels().iter().filter(|&&x| x == 1).count());
    println!("First few rows of the result:");
    ...
    for i in 0..std::cmp::min(5, svm_result.row_count()) {
        if let (Ok(Some(x)), Ok(Some(y)), Ok(Some(decision_value)), Ok(Some(anomaly))) = (
            ...
            println!("Row {}: x={:.4}, y={:.4}, decision={:.4}, anomaly={}", 
    ...
    for i in 0..n_samples {
        ...
        if if_label == lof_label && lof_label == svm_label {
            ...
            if if_label == 1 && all_agree <= 5 { // Display only the first 5
                // Instead of accessing values, get values from the dataframe
                if let (Ok(Some(x)), Ok(Some(y))) = (
                    df.column("x").unwrap().as_float64().unwrap().get(i),
                    df.column("y").unwrap().as_float64().unwrap().get(i)
                ) {
                    println!("Sample {} is detected as an anomaly by all algorithms: x={:.2}, y={:.2}", 
                        i, x, y);
                }
            }
        }
    ...
    println!("Agreement rate of all algorithms: {:.1}%", 100.0 * all_agree as f64 / n_samples as f64);
    println!("Agreement rate of Isolation Forest and LOF: {:.1}%", 100.0 * if_lof_agree as f64 / n_samples as f64);
    println!("Agreement rate of Isolation Forest and SVM: {:.1}%", 100.0 * if_svm_agree as f64 / n_samples as f64);
    println!("Agreement rate of LOF and SVM: {:.1}%", 100.0 * lof_svm_agree as f64 / n_samples as f64);
    ...
    let calc_metrics = |algorithm_name: &str, labels: &[i64], true_labels: &[i64]| {
        ...
        println!("{}: Precision={:.1}%, Recall={:.1}%, F1={:.3}, Accuracy={:.1}%", 
