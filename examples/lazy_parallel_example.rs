...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    result_df.add_column("Department", Column::String(StringColumn::new(result_depts)))?;
    result_df.add_column("Total Salary", Column::Float64(Float64Column::new(result_totals)))?;
    result_df.add_column("Average Salary", Column::Float64(Float64Column::new(result_avgs)))?;
    ...
    println!("The LazyFrame approach is {:.2} times faster than the standard approach", speedup);
...
fn generate_large_dataframe(rows: usize) -> Result<OptimizedDataFrame, Box<dyn Error>> {
    ...
    for i in 0..rows {
        ...
        ages.push(rng.gen_range(20..60)); // Age
        depts.push(departments[rng.gen_range(0..departments.len())].clone()); // Department
