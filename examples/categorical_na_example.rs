...
fn main() -> Result<()> {
    ...
    println!("Is 'Evaluation' column categorical: {}", df.is_categorical("Evaluation"));
    
    // Explicitly handle errors
    match df.get_categorical("Evaluation") {
        Ok(cat_col) => println!("Categories of 'Evaluation' column: {:?}", cat_col.categories()),
    ...
    println!("First value of 'Evaluation' column: {:?}", df_loaded.get_column("Evaluation").unwrap().values()[0]);
