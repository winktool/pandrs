...
fn main() -> Result<(), PandRSError> {
    ...
    println!("\nDataFrame with Polynomial Features: {:?} columns", poly_df.column_names().len());
    
    // 2. Binning (Discretization)
    let mut binner = Binner::new_uniform(vec!["value1".to_string()], 4);
    let binned_df = binner.fit_transform(&opt_df)?;
    
    println!("\nDataFrame after Binning: {:?} columns", binned_df.column_names().len());
        ...
        if let Some(float_col) = value1_view.as_float64() {
            ...
            for i in 0..col_len {
                if let Ok(Some(val)) = float_col.get(i) {
                    ...
                    if rng.random_bool(0.2) {  // 20% chance of NA
                        values.push(None);
                    } else {
    ...
    println!("\nDataFrame with Missing Values: {:?} rows", na_opt_df.row_count());
    
    // Display DataFrame with missing values (simplified)
    println!("\nDataFrame with Missing Values: {:?} columns", na_opt_df.column_names().len());
    
    // Impute with mean value
    let mut imputer = Imputer::new(vec!["value1".to_string()], ImputeStrategy::Mean);
    let imputed_df = imputer.fit_transform(&na_opt_df)?;
    
    println!("\nDataFrame with Missing Values Imputed by Mean: {:?} columns", imputed_df.column_names().len());
    ...
    println!("\nFeatures Selected Based on Variance: {:?} columns", selected_df.column_names().len());
    ...
    println!("DataFrame after Pipeline Transformation: {:?} columns", transformed_df.column_names().len());
