...
pub(crate) fn linear_regression_impl(
    ...
    if x_columns.is_empty() {
        return Err(Error::InvalidOperation("Regression analysis requires at least one predictor variable".into()));
    }
    ...
    for &x_col in x_columns {
        ...
        if x_values.len() != n {
            return Err(Error::DimensionMismatch(
                format!("Regression analysis: Column lengths do not match: y={}, {}={}", n, x_col, x_values.len())
            ));
        }
...
fn calculate_std_errors(
    ...
    if df <= 0 {
        return Err(Error::InsufficientData("Degrees of freedom is 0 or negative. More data points needed.".into()));
    }
