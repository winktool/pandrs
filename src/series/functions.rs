...
pub mod common_functions {
    ...
    where 
        T: Debug + Clone + std::iter::Sum + std::ops::Div<Output = T> + num_traits::NumCast + Default,
    {
        if values.is_empty() {
            return Err(crate::core::error::Error::EmptyData("Cannot calculate mean of an empty Series".to_string()));
        }
        ...
        let count = match num_traits::cast(values.len()) {
            Some(n) => n,
            None => {
                return Err(crate::core::error::Error::Cast("Cannot cast length to numeric type".to_string()))
            }
        };
