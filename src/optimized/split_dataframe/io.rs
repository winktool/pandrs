...
impl OptimizedDataFrame {
    ...
    pub fn to_parquet<P: AsRef<Path>>(&self, path: P, compression: Option<ParquetCompression>) -> Result<()> {
        ...
        let file = File::create(path.as_ref())
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to create Parquet file: {}", e))))?;
        
        // Create Arrow writer and write data
        let mut writer = ArrowWriter::try_new(file, schema_ref, Some(props))
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to create Parquet writer: {}", e))))?;
        
        // Write record batch
        writer.write(&batch)
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to write record batch: {}", e))))?;
        
        // Close the file
        writer.close()
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to close Parquet file: {}", e))))?;
    ...
    pub fn from_parquet<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let file = File::open(path.as_ref())
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to open Parquet file: {}", e))))?;
        
        // Create Arrow's Parquet reader
        let builder = ParquetRecordBatchReaderBuilder::try_new(file)
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to parse Parquet file: {}", e))))?;
        ...
        let reader = builder.build()
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to read Parquet file: {}", e))))?;
        
        // Read all record batches
        let mut all_batches = Vec::new();
        for batch_result in reader {
            let batch = batch_result
                .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to read record batch: {}", e))))?;
                ...
                DataType::Int64 => {
                    ...
                    for batch in &all_batches {
                        let array = batch.column(col_idx).as_any().downcast_ref::<Int64Array>()
                            .ok_or_else(|| Error::Cast(format!("Could not convert column '{}' to Int64Array", col_name)))?;
                ...
                DataType::Float64 => {
                    ...
                    for batch in &all_batches {
                        let array = batch.column(col_idx).as_any().downcast_ref::<Float64Array>()
                            .ok_or_else(|| Error::Cast(format!("Could not convert column '{}' to Float64Array", col_name)))?;
                ...
                DataType::Boolean => {
                    ...
                    for batch in &all_batches {
                        let array = batch.column(col_idx).as_any().downcast_ref::<BooleanArray>()
                            .ok_or_else(|| Error::Cast(format!("Could not convert column '{}' to BooleanArray", col_name)))?;
                ...
                DataType::Utf8 | DataType::LargeUtf8 => {
                    ...
                    for batch in &all_batches {
                        let array = batch.column(col_idx).as_any().downcast_ref::<StringArray>()
                            .ok_or_else(|| Error::Cast(format!("Could not convert column '{}' to StringArray", col_name)))?;
    ...
    pub fn from_excel<P: AsRef<Path>>(
        ...
        let mut workbook: Xlsx<BufReader<File>> = open_workbook(path.as_ref())
            .map_err(|e| Error::Io(crate::error::io_error(format!("Could not open Excel file: {}", e))))?;
        ...
        let range = workbook.worksheet_range(&sheet_name)
            .map_err(|e| Error::Io(crate::error::io_error(format!("Could not read sheet '{}': {}", sheet_name, e))))?;
    ...
    pub fn to_excel<P: AsRef<Path>>(
        ...
        let mut workbook = Workbook::create(path.as_ref()
            .to_str()
            .ok_or_else(|| Error::Io(crate::error::io_error("Could not convert file path to string")))?);
        ...
        workbook.close()
            .map_err(|e| Error::Io(crate::error::io_error(format!("Could not save Excel file: {}", e))))?;
    ...
    pub fn from_sql<P: AsRef<Path>>(query: &str, db_path: P) -> Result<Self> {
        ...
        let conn = Connection::open(db_path)
            .map_err(|e| Error::Io(crate::error::io_error(format!("Could not connect to database: {}", e))))?;
        
        // Prepare query
        let mut stmt = conn.prepare(query)
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to prepare SQL query: {}", e))))?;
        ...
        let mut rows = stmt.query([])
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to execute SQL query: {}", e))))?;
        
        // Process each row of data
        while let Some(row) = rows.next()
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to get SQL query results: {}", e))))? {
            
            for (idx, name) in column_names.iter().enumerate() {
                let value: Option<String> = row.get(idx)
                    .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to get row data: {}", e))))?;
    ...
    pub fn to_sql<P: AsRef<Path>>(&self, table_name: &str, db_path: P, if_exists: &str) -> Result<()> {
        ...
        let table_exists = conn.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name=?")
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to prepare table check query: {}", e))))?
            .exists(params![table_name])
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to check if table exists: {}", e))))?;
        
        // Handle table based on if_exists parameter
        if table_exists {
            match if_exists {
                "fail" => {
                    return Err(Error::Io(crate::error::io_error(format!("Table '{}' already exists", table_name))));
                },
                "replace" => {
                    // Drop table and create new one
                    conn.execute(&format!("DROP TABLE IF EXISTS {}", table_name), [])
                        .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to drop table: {}", e))))?;
                ...
                _ => {
                    return Err(Error::Io(crate::error::io_error(format!("Unknown if_exists value: {}", if_exists))));
                }
            }
        } else {
        ...
        let tx = conn.transaction()
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to start transaction: {}", e))))?;
        ...
        for row_idx in 0..self.row_count {
            ...
            let mut stmt = tx.prepare(&insert_sql)
                .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to prepare INSERT statement: {}", e))))?;
            ...
            stmt.execute(params.as_slice())
                .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to insert data: {}", e))))?;
        }
        
        // Commit transaction
        tx.commit()
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to commit transaction: {}", e))))?;
    ...
    fn create_table_from_df(&self, conn: &Connection, table_name: &str) -> Result<()> {
        ...
        conn.execute(&create_sql, [])
            .map_err(|e| Error::Io(crate::error::io_error(format!("Failed to create table: {}", e))))?;
