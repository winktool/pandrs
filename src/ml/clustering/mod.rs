            ...
            for col_name in feature_columns {
                ...
                if let Ok(col_f64) = data.get_column::<f64>(col_name) {
                    ...
                    return Err(Error::InvalidInput(
                        format!("Column {} is not numeric", col_name)
                    ));
                }
            }
...
impl ModelEvaluator for KMeans {
    fn evaluate(&self, test_data: &DataFrame, _test_target: &str) -> Result<ModelMetrics> {
        ...
        if let Some(labels) = &self.labels {
            if let Some(centroids) = &self.centroids {
                let silhouette = compute_silhouette(test_data, labels, centroids, &self.feature_columns)?;
    ...
    fn cross_validate(&self, _data: &DataFrame, _target: &str, _folds: usize) -> Result<Vec<ModelMetrics>> {
        ...
        Err(Error::InvalidOperation("Cross-validation is not applicable for K-means clustering".into()))
    }
}
...
fn compute_silhouette(data: &DataFrame, labels: &[usize], centroids: &[Vec<f64>], feature_columns: &Option<Vec<String>>) -> Result<f64> {
...
impl UnsupervisedModel for AgglomerativeClustering {
    ...
    fn transform(&self, _data: &DataFrame) -> Result<DataFrame> {
        ...
        Err(Error::InvalidOperation("AgglomerativeClustering does not support transform".into()))
    }
}
...
impl ModelEvaluator for AgglomerativeClustering {
    ...
    fn cross_validate(&self, _data: &DataFrame, _target: &str, _folds: usize) -> Result<Vec<ModelMetrics>> {
        Err(Error::InvalidOperation("Cross-validation is not applicable for hierarchical clustering".into()))
    }
}
...
impl UnsupervisedModel for DBSCAN {
    ...
    fn transform(&self, _data: &DataFrame) -> Result<DataFrame> {
        ...
        Err(Error::InvalidOperation("DBSCAN does not support transform".into()))
    }
}
...
impl ModelEvaluator for DBSCAN {
    ...
    fn cross_validate(&self, _data: &DataFrame, _target: &str, _folds: usize) -> Result<Vec<ModelMetrics>> {
        Err(Error::InvalidOperation("Cross-validation is not applicable for DBSCAN clustering".into()))
    }
}
