...
fn linear_regression_gpu(
    ...
    let adj_r_squared = 1.0 - (1.0 - r_squared) * ((n_samples - 1) as f64) / ((n_samples - n_features - 1) as f64);
    ...
    for iter in 0..max_iter {
        ...
        for i in 0..n_samples {
            ...
            for c in 0..k {
                ...
                let dist_squared: f64 = point.iter()
                    .zip(centroid.iter())
                    .map(|(&p, &c)| (p - c).powi(2))
                    .sum();
...
fn invert_matrix(matrix: &Array2<f64>) -> Result<Array2<f64>> {
    ...
    if n != m {
        return Err(Error::Dimension(format!("Matrix must be square for inversion, got {:?}", (n, m))));
    }
