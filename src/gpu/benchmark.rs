...
use crate::gpu::{get_gpu_manager, GpuDeviceStatus, GpuConfig};
...
impl BenchmarkSummary {
    ...
    pub fn new(cpu_result: BenchmarkResult, gpu_result: Option<BenchmarkResult>) -> Self {
        ...
        // Calculate speedup
        let speedup = gpu_result.as_ref().map(|gpu| {
            cpu_result.time.as_secs_f64() / gpu.time.as_secs_f64()
        });
...
impl GpuBenchmark {
    ...
    pub fn benchmark_matrix_multiply(&mut self, m: usize, n: usize, k: usize) -> Result<&BenchmarkSummary> {
    ...
    pub fn benchmark_linear_regression(&mut self, rows: usize, cols: usize) -> Result<&BenchmarkSummary> {
        // Create test DataFrame
        let mut df = DataFrame::new();
        ...
        let feature_cols: Vec<&str> = df.column_names().iter()
            .filter(|&name| name != "y")
            .map(|s| s.as_str())
            .collect();
    ...
    pub fn benchmark_rolling_window(&mut self, size: usize, window_size: usize) -> Result<&BenchmarkSummary> {
        // Create test Series
        ...
        // Benchmark CPU implementation
        let cpu_start = Instant::now();
        let _cpu_result = series.rolling(window_size, window_size / 2, crate::temporal::window::WindowOperation::Mean, false)?;
        let cpu_time = cpu_start.elapsed();
    ...
    pub fn get_summary(&self) -> String {
        ...
        if self.benchmarks.is_empty() {
            ...
            let max_op_len = self.benchmarks.iter()
                .map(|b| format!("{}", b.operation).len())
                .max()
                .unwrap_or(0);
                
            let max_size_len = self.benchmarks.iter()
                .map(|b| b.data_size.len())
                .max()
                .unwrap_or(0);
            
            // Add table header
            output.push_str(&format!("{:<width_op$} | {:<width_size$} | {:>10} | {:>10} | {:>8}\n",
                "Operation",
                "Data Size",
                "CPU (ms)",
                "GPU (ms)",
            ...
            for benchmark in &self.benchmarks {
                let gpu_time = benchmark.gpu_result.as_ref()
                    .map(|r| format!("{:.2}", r.time_ms()))
                    .unwrap_or("N/A".to_string());
                    
                let speedup = benchmark.speedup
                    .map(|s| format!("{:.2}x", s))
                    .unwrap_or("N/A".to_string());
                
                output.push_str(&format!("{:<width_op$} | {:<width_size$} | {:>10.2} | {:>10} | {:>8}\n",
