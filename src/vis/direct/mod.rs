...
use crate::vis::config::{PlotSettings, OutputType};
#[cfg(feature = "visualization")]
...
pub trait SeriesPlotExt<T> {
    ...
    fn histogram<P: AsRef<Path>>(&self, path: P, bins: Option<usize>, title: Option<&str>) -> Result<()>;
    ...
    fn plot_svg<P: AsRef<Path>>(&self, path: P, plot_kind: PlotKind, title: Option<&str>) -> Result<()>;
}
...
pub trait DataFramePlotExt {
    ...
    fn plot_column<P: AsRef<Path>>(&self, column: &str, path: P, title: Option<&str>) -> Result<()>;
    ...
    fn scatter_plot<P: AsRef<Path>>(&self, column: &str, path: P, title: Option<&str>) -> Result<()>;
    ...
    fn box_plot<P: AsRef<Path>>(&self, value_column: &str, category_column: &str, path: P, title: Option<&str>) -> Result<()>;
    
    /// Create a scatter plot between two columns
    fn scatter_xy<P: AsRef<Path>>(&self, x_column: &str, y_column: &str, path: P, title: Option<&str>) -> Result<()>;
    
    /// Create a line plot for multiple columns
    fn multi_line_plot<P: AsRef<Path>>(&self, columns: &[&str], path: P, title: Option<&str>) -> Result<()>;
    
    /// Save the plot as SVG format
    fn plot_svg<P: AsRef<Path>>(&self, column: &str, path: P, plot_kind: PlotKind, title: Option<&str>) -> Result<()>;
}
    ...
    T: Clone + Copy + Into<f64> + std::fmt::Debug,
{
    fn plot_to<P: AsRef<Path>>(&self, _path: P, _title: Option<&str>) -> Result<()> {
        Err(crate::error::PandRSError::FeatureNotAvailable("Visualization feature is not enabled. Recompile with --feature visualization".to_string()))
    }
    ...
    fn histogram<P: AsRef<Path>>(&self, _path: P, _bins: Option<usize>, _title: Option<&str>) -> Result<()> {
        Err(crate::error::PandRSError::FeatureNotAvailable("Visualization feature is not enabled. Recompile with --feature visualization".to_string()))
    }
    ...
    fn plot_svg<P: AsRef<Path>>(&self, _path: P, _plot_kind: crate::vis::config::PlotKind, _title: Option<&str>) -> Result<()> {
        Err(crate::error::PandRSError::FeatureNotAvailable("Visualization feature is not enabled. Recompile with --feature visualization".to_string()))
    }
}
...
impl DataFramePlotExt for DataFrame {
    ...
    fn plot_svg<P: AsRef<Path>>(&self, _column: &str, _path: P, _plot_kind: crate::vis::config::PlotKind, _title: Option<&str>) -> Result<()> {
        Err(crate::error::PandRSError::FeatureNotAvailable("Visualization feature is not enabled. Recompile with --feature visualization".to_string()))
    }
}
